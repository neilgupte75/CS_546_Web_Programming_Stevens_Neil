...
let exportedMethods = {
  ...
  async addBand(bandName, bandMembers, yearFormed, genres, recordLabel) {
    if(!bandName) throw new Error("Name cannot be empty");

    if(!bandMembers) throw new Error("Band members cannot be empty");
    if(!yearFormed) throw new Error("Year formed cannot be empty");
    if(!recordLabel) throw new Error("Record Label cannot  be empty");
    if(!genres) throw new Error("Record Label cannot  be empty");
    if(bandName==null||bandName==undefined) throw new Error("Cannot be undef or null")
    if(recordLabel==null||bandName==undefined) throw new Error("Cannot be undef or null")
    if(!bandMembers) throw new Error("Band members cannot be empty");
    if(typeof(bandName)!=="string") throw new Error("The name should be in string format");
    if(typeof(yearFormed)!=="number") throw new Error("The year should be in number format");
    if(Array.isArray(bandMembers)==0) throw new Error("The band not in proper format");
    if(bandMembers.length<1) throw new Error("The bandmembers should have one entry")
    if(Array.isArray(genres)==0) throw new Error("The genre not in proper format");
    if(genres.length<1) throw new Error("The genre should have one entry")
    if(yearFormed>2019 || yearFormed<1900) throw new Error("Year is not right")
    /*
   ...
   return await this.getBandByName(newInsertInformation.insertedId);
  ...
  if(typeof(bandName)==="number") throw new Error("The id should be in string format");
  ...
  async searchBandByGenre(genre) {
    ...
    */
   if(!genre){
     throw "You must input the genre"
    }
    const bandCollection = await bands();
    return await bandCollection.find({ genre: { $in: genre } }).toArray();
